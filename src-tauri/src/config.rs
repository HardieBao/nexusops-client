...
///   且不一定等于用户目录，可能导致配置目录变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
/// 为了让 Windows CI/本地测试能稳定隔离真实用户数据，可通过
/// `NEXUSOPS_CLIENT_TEST_HOME` 显式覆盖；旧的 `CC_SWITCH_TEST_HOME` 继续用于上游测试。
pub fn get_home_dir() -> PathBuf {
    for variable in ["NEXUSOPS_CLIENT_TEST_HOME", "CC_SWITCH_TEST_HOME"] {
...
/// NexusOps uses its own configuration root so it can coexist with upstream CC Switch.
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    get_home_dir().join(".nexusops-client")
}
