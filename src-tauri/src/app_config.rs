...
impl MultiAppConfig {
    ...
    pub fn load() -> Result<Self, AppError> {
        ...
        if is_v1 {
            return Err(AppError::localized(
                ...
                "检测到旧版 v1 配置格式。当前版本已不再支持运行时自动迁移。\n\n解决方案：\n1. 使用兼容版本进行一次性迁移\n2. 或手动编辑 ~/.nexusops-client/config.json，将顶层结构调整为：\n   {\"version\": 2, \"claude\": {...}, \"codex\": {...}, \"mcp\": {...}}\n\n",
                "Detected legacy v1 config. Runtime auto-migration is no longer supported.\n\nSolutions:\n1. Use a compatible version for a one-time migration\n2. Or manually edit ~/.nexusops-client/config.json to adjust the top-level structure:\n   {\"version\": 2, \"claude\": {...}, \"codex\": {...}, \"mcp\": {...}}\n\n",
            ));
        }
    ...
    pub fn save(&self) -> Result<(), AppError> {
        let config_path = get_app_config_path();
        // 先备份旧版（若存在）到应用配置目录的 config.json.bak，再写入新内容
        if config_path.exists() {
