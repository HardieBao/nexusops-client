...
核心统计补验：[新旧用量入口一致性](evidence/usage-entry-parity-20260912.md)。代理关闭时，真实本机 4 条模型记录在新旧入口的汇总、Codex 筛选与逐行明细一致，刷新偏好可共用。另补齐四语言本机估算口径说明，类型 / 构建及 6 项相关测试通过；新说明待后续包实际显示检查。
